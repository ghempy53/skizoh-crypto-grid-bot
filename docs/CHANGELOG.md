...
| **Volatility-scaled spacing cap** (`max_spacing_cap_percent`, default 1.25%; effective cap = max(config, 30% of 24h range)) | Regime blend pushed spacing to 1.5–2.3% (Swing 2.7% weight) while true daily ranges were 1–3.7% — orders sat unfillable | Spacing clamped in calm markets (tight grids cycle nearly free at 0% maker); cap rises automatically with measured volatility so an 8–12% range day still gets a 2.4–3.6% grid |
